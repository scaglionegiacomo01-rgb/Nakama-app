...
import {
  Users, Bell, Camera, Car, CalendarDays, AlertTriangle, CheckCircle2,
  Plus, ClipboardCheck, UserCog, Download,
} from "lucide-react";
...
function AdminOverviewPage() {
  ...
  return (
    <AdminShell title="Operations dashboard" description="Run trips, keep the crew safe, and make sure nobody gets left behind.">
      <OverviewSection />
    </AdminShell>
  );
}
...
function QuickAction({ to, icon: Icon, label }: { to: string; icon: React.ComponentType<{ className?: string }>; label: string }) {
  return (
    <Link to={to} className="rounded-xl border border-border bg-card hover:bg-secondary p-3 text-sm font-medium inline-flex items-center gap-2">
