...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex">
      {user && <DesktopSidebar isAdmin={isAdmin} />}
      <div className="flex-1 min-w-0 flex flex-col">
        {user && (
          <div className="hidden lg:flex items-center gap-4 h-[68px] shrink-0 px-8 border-b border-border">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                navigate({ to: "/trips", search: { q: topbarQuery || undefined } });
              }}
              className="flex-1 max-w-[380px]"
            >
              <div className="h-10 rounded-[13px] bg-secondary/60 border border-border flex items-center gap-2.5 px-3.5">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <Globe className="w-[17px] h-[17px]" strokeWidth={1.75} />
                  {lang.toUpperCase()}
                </button>
              </DropdownMenuTrigger>
            ...
            <Link
              to="/community"
              className="h-10 px-[18px] rounded-[13px] bg-gradient-to-br from-[oklch(0.45_0.19_5)] to-[oklch(0.36_0.15_355)] text-white text-[13.5px] font-semibold inline-flex items-center whitespace-nowrap"
            >
              {t("nav.propose_trip")}
            </Link>
          </div>
        ...
        <header
          className={cn(
            user && "lg:hidden",
            "nakama-glass sticky top-0 z-40 border-b border-white/10",
          )}
        >
